...
def test(N: int, num_runs: int, success_threshold: float) -> None:
    ...
    with open(os.path.join("test", f"expected_stats_{N}.txt"), "r") as f:
        mean_epsilon_values, std_epsilon_values = ([float(x) for x in line.split()] for line in f.read().split("\n"))
...
def compute_statistics(N: int, num_runs: int) -> None:
    print(f"Computing statistics for tiny_md with {N} particles after {num_runs} runs...")
    ...
    with open(os.path.join("test", f"expected_stats_{N}.txt"), "w") as f:
        ...
        mean_epsilon_values = np.mean(mean_diffs, axis=0) + 2 * np.std(mean_diffs, axis=0)
        std_epsilon_values = np.mean(std_diffs, axis=0) + 2 * np.std(std_diffs, axis=0)
